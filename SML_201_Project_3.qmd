...
pred1 = coef1[2:3] * test[, c("hour", "weekend")] |>
  mutate(pred = coef1[1] + hour + weekend)
...
pred2 = coef2[2:3] * test[, c("hour", "weekend")] |>
  mutate(pred = coef2[1] + hour + weekend)
...
> Looking at the outputs of both models in A3, we observe that the ridge regression model (model2) achieved a lower MSE compared to the lasso ridge model (model1). This suggests that the ridge regression model performed better in predicting the number of plays when hours and weekend are used as the predictors. Since Ridge regression generally handles multicollinearity and distributes coefficient weights more evenly than Lasso regression, we can infer that both the predictors (weekend and hours) might be influential in contributing to the model. From the Hypothesis test we achieved a p-value of 0.03167 which is less than the significance value of alpha = 0.05, which means we achieved a result that was not significant enough. Thus, we fail to reject the null hypothesis that “Love Story” was played significantly more.
