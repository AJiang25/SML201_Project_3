...
least_plays = swiftie_least_plays_cruel$total_plays[1]

least_cruel_summer_swiftie = swiftie_least_plays_cruel |>
  filter(total_plays == least_plays)

least_cruel_summer_swiftie$swiftie
```
...
```{r a4}
set.seed(our_seed)
swiftie_samples <- sample(ts_plays$swiftie, 25, replace=FALSE)

love_story <- ts_plays |>
  filter(swiftie %in% swiftie_samples & song == 'Love Story') |>
  group_by(swiftie) |>
  count(song)
you_belong_with_me <- ts_plays |>
  filter(swiftie %in% swiftie_samples & song == 'You Belong With Me') |>
  group_by(swiftie) |>
  count(song)
love_story
you_belong_with_me
t.test(love_story$n, you_belong_with_me$n, var.equal = FALSE, alternative = "greater")

paste("Reject Love Story plays more than You Belong With Me because p_val > 0.05")
...
> Looking at the outputs of both models, we observe that they achieved identical MSE values of approximately 0.04073, which suggests that both models are suitable. Both models show 0 nonzero coefficients, which suggests that neither the hour or weekend features were strong predictors of the play counts. The identical performance suggests that there might be a weak relationship between the predictors (hour and weekend) and the number of plays or there might be other more important features that are not included in the models. From the Hypothesis test we achieved a p-value of 0.2566 which is greater than the typical threshold of alpha = 0.05, which means we achieved a result that was significant enough to reject the null hypothesis that “Love Story” was played significantly more.
