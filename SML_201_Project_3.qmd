...
> Looking at the outputs of both models in A3, we observe that the lasso regression model (model1) achieved a lower MSE compared to the ridge ridge model (model2). This suggests that the lasso regression model performed better in predicting the number of plays when hours and weekend are used as the predictors. Since lasso regression generally shrinks less informative predictors to zero, this suggests that either “weekend” or “hour” has relatively little influence on predicting the number of plays compared to the other. From the Hypothesis test we achieved a p-value of 0.03167 which is less than the significance value of alpha = 0.05, which means we achieved a result that was not significant enough. Thus, we fail to reject the null hypothesis that “Love Story” was played significantly more.
